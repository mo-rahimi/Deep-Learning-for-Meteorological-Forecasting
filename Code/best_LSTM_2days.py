...
192               |192               |units_3
0.4               |0.4               |dropout_3
0.00066498        |0.00066498        |learning_rate
'''
def create_lstm_model(input_shape, num_outputs):
    model = Sequential()
    model.add(LSTM(160, return_sequences=True, input_shape=input_shape, activation='tanh'))
    model.add(Dropout(0.3))
    model.add(LSTM(192, return_sequences=True, activation='tanh'))
    model.add(Dropout(0.5))
    model.add(LSTM(192, activation='tanh'))
    model.add(Dropout(0.4))
    model.add(Dense(num_outputs, activation='linear'))

    optimizer = Adam(learning_rate=0.00066498)
...
model.save("lstm_2days_prediction_try4.h5")

import pickle
with open("history_2days_try4.pkl", "wb") as file:
